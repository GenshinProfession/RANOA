...
test("does not rewrite the repository agent instructions during local development", async () => {
  const config = await createJiti(import.meta.url).import("../next.config.ts", { default: true });

  assert.equal(config.agentRules, false);
});

test("keeps the Next.js development badge out of the themed interface", async () => {
  const config = await createJiti(import.meta.url).import("../next.config.ts", { default: true });
